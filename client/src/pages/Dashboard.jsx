...
const Dashboard = () => {
  ...
  const editTitle = async (event) => {
    try {
        event.preventDefault()
        const {data} = await api.put(`resumes/update/${editResumeId}`,{resumeData:{title}},{headers: {Authorization:token}})
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
          {allResumes.map((resume,index) => {
            ...
            return (
              <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)}  className="relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-between p-4 rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer" style={{background:`linear-gradient(135deg, ${baseColor} 10, ${baseColor}40)`,borderColor:baseColor+'40'}}>
        ...
        {showCreateResume && (
          <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)}  className="fixed inset-0 bg-black/70 backdrop-blur flex bg-opacity-50 z-10  flex items-center justify-center">
        ...
        {showUploadResume && (
          ...
          <div onClick={e=>e.stopPropagation()} className="relative bg-slate-50 border shadow-mb rounded-lg w-full max-w-sm p-6">
            ...
            <div>
              ...
              <input type="file" id="resume-input" accept=".pdf,.doc,.docx" className="hidden" onChange={(e) => setResume(e.target.files[0])} />
            </div>
            <button className="w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors">
